...
clc(); compile("selectivity.cpp")
dyn.load(dynlib("selectivity"))
...
parameters <- list(log_shape_selectivity = log(5),                # Shape parameter for cumulative gamma distribution function.
                   log_scale_selectivity = log(10))               # Scale parameter for cumulative gamma distribution function.

# Create TMB model object:
obj <- MakeADFun(data = data, parameters = parameters, DLL = "selectivity")

plot(data$x, obj$report()$p_selectivity, type = "l", lwd = 2)
