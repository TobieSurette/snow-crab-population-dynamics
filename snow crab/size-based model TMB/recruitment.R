...
parameters <- list(mu_recruitment = 10,                           # Size of recruitment instar.
                   log_sigma_recruitment= log(0.8),               # Error for recruitment instar.
                   R = 1:10)                                      # Recruitment scaling values. 

# Create TMB model object:
obj <- MakeADFun(data = data, parameters = parameters, DLL = "recruitment")

# Plot results:
plot(data$x, obj$report()$p_recruitment, type = "l", lwd = 2)
   
image(obj$report()$N_recruitment)
