...
clc(); compile("mortality.cpp")
dyn.load(dynlib("mortality"))
...
parameters <- list(xp_F = 98,                             # Exploitation rate switching point.
                   log_window_F = -1,                     # Exploitation switching point window.
                   log_mu_F = log(0.4),                   # Global exploitation rate.      
                   log_dev_F = 0.1 * rnorm(10),           # Annual deviations exploitation rate.
                   log_mu_M = log(0.3),                   # Global natural mortality rate.
                   log_dev_M = 0.1 * rnorm(10))           # Annual deviations natural mortality rate.

# Create TMB model object:
obj <- MakeADFun(data = data, parameters = parameters, DLL = "mortality")

# Plot results:
plot(data$x, obj$report()$F[1,], type = "l", lwd = 2)
image(obj$report()$F)

plot(data$x, obj$report()$M[1,], type = "l", lwd = 2)
