...
template<class Type> Type objective_function<Type>::operator()(){
   ...
   for (int i = 0; i < ni; i++){ 
      ...
      Type selectivity = Type(1) / (Type(1) + exp(-selectivity_slope * (x_imm[i] - selectivity_x50)));
      
      for (int k = 0; k < n_instar; k++){
         eta_imm[i] += year_effect[year_imm[i]] * 
                       selectivity *
                      (n_imm(k,year_imm[i]) + (1-selectivity_skp) * n_skp(k,year_imm[i])) * 
   ...
   for (int i = 0; i < nm; i++){ 
      ...
      Type selectivity = Type(1) / (Type(1) + exp(-selectivity_slope * (x_mat[i] - selectivity_x50))); 
      
      // Define fishing size-selectivity curve for recruitment and residuals:
      Type selectivity_rec = fishing_effect_rec / (Type(1) + exp(-selectivity_slope_fishing * (x_mat[i] - selectivity_x50_fishing)));   
      Type selectivity_res = fishing_effect_res / (Type(1) + exp(-selectivity_slope_fishing * (x_mat[i] - selectivity_x50_fishing)));   
